...
const Navbar = () => {
    ...
    const {setShowSearch ,navigate, getCartCount, token, setToken, setCartItems, clearCart} = useContext(ShopContext);
    const logout = () =>{
        navigate('/login')
        localStorage.removeItem('token')
        clearCart(); // Use the clearCart function
        setToken('')
    }
    ...
    return (
        <div className='flex items-center justify-between py-5 font-medium'>
            ...
            {/* Search , profile ,and cart part */}
            <div className='flex items-center gap-6'>
                <Link to= '/collection'><img onClick={()=> setShowSearch(true)} src={assets.search_icon} className='w-5 cursor-pointer' alt="Search Icon" /> </Link>
